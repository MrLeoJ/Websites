document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayCodes(codes) {
     ...
     function handleToggleInteractions() {
        const includeNumbersCheckbox = document.getElementById('includeNumbers');
        const numbersOnlyCheckbox = document.getElementById('numbersOnly');
        const includeSymbolsCheckbox = document.getElementById('includeSymbols');

        numbersOnlyCheckbox.addEventListener('change', () => {
            if (numbersOnlyCheckbox.checked) {
                includeNumbersCheckbox.checked = true; // Automatically check "Include Numbers"
                includeNumbersCheckbox.disabled = true;  // Disable "Include Numbers"
                includeSymbolsCheckbox.checked = false; // Uncheck "Include Symbols"
                includeSymbolsCheckbox.disabled = true; // Disable
            } else {
                includeNumbersCheckbox.disabled = false; // Enable "Include Numbers"
                includeSymbolsCheckbox.disabled = false; // Enable
            }
        });

        includeNumbersCheckbox.addEventListener('change', () => {
            if (!includeNumbersCheckbox.checked) {
                numbersOnlyCheckbox.checked = false; // if includeNumbers is unchecked also uncheck numbersOnly
            }
        });
    }
