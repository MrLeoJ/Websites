...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateAllCardWatchlistIcons = () => {
        ...
        cards.forEach(btn => {
            const card = btn.closest('.media-card, .filmography-detail-container');
            if (card) {
    ...
    const toggleWatchlistAction = async (id, type, button) => {
        ...
        try {
            ...
            if (isInWatchlist) {
                ...
                if ((isWatchlistMode || isWatchingMode) && button) {
    ...
    const _setModalContent = (content, { isTrailer = false, isNavigable = false } = {}) => {
        ...
        // Handle Enter key on focusable items within the modal
        contentWrapper.addEventListener('keydown', e => {
            if (e.key === 'Enter' && e.target.matches('.cast-member, .filmography-item')) {
                e.preventDefault();
                e.target.click();
            }
        });
    ...
    window.showCast = async (id, type) => {
        ...
        try {
            ...
            castToDisplay.forEach((member, index) => {
                ...
                castGridHTML += `
                    <div class="cast-member" tabindex="0" onclick="showFilmography(${member.id}, '${escapedName}')">
        ...
        try {
            ...
            const createGridHTML = (items) => {
                ...
                items.forEach(item => {
                    ...
                    gridHTML += `
                         <div class="filmography-item" tabindex="0" onclick="showFilmographyItemDetails(${item.id}, '${item.media_type}')">
    ...
    document.addEventListener('keydown', (e) => {
        ...
        // TV Back button is often 'Backspace'. We don't want to interfere if user is typing.
        if (e.key === 'Backspace' && isTextInputFocused) {
            return;
        }
        ...
        if (e.key === 'Escape' || e.key === 'Backspace') {
            if (modalHistory.length > 0) {
                ...
                goBackInModal();
                 ...
                 // Keep original behavior: only Escape clears search
                searchInput.value = '';
                searchTerm = '';
                handleSearch();
            }
