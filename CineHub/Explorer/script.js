...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const _setModalContent = (content, { isTrailer = false, isNavigable = false } = {}) => {
        ...
        if(!isNavigable) {
            const closeBtn = modal.querySelector('.modal-close-btn, .modal-close-btn-trailer');
            if(closeBtn) contentWrapper.appendChild(closeBtn);
        }
        ...
        // Focus trap for accessibility and TV navigation
    ...
    const loadCardDetailsIfNeeded = async (card) => {
        ...
        try {
            ...
            const firstButton = content.querySelector('.details-btn');
            if (firstButton) {
                firstButton.focus();
            }
    ...
    window.showCast = async (id, type) => {
        ...
        try {
            ...
            castToDisplay.forEach((member, index) => {
                ...
                castGridHTML += `
                    <div class="cast-member" role="button" tabindex="0" onclick="showFilmography(${member.id}, '${escapedName}')">
    ...
    window.showFilmography = async (personId, name) => {
        ...
        try {
            ...
            filmography.forEach(item => {
                ...
                filmographyGridHTML += `
                     <div class="filmography-item" role="button" tabindex="0" onclick="showFilmographyItemDetails(${item.id}, '${item.media_type}')">
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            if (modalHistory.length > 0) {
                goBackInModal();
            } else if (currentlyExpandedCard) {
                const cardToCollapse = currentlyExpandedCard;
                cardToCollapse.classList.remove('expanded');
                currentlyExpandedCard = null;
                cardToCollapse.focus();
